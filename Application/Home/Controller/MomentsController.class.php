...
class MomentsController extends CommonController
{
    ...
    public function searchUser()
    {
        ...
        foreach ($this->obj->getUserId($user_name, $search_name) as $k => $val) {
            ...
            if ($result) {
                //$list['is_follow'] = 1; //已关注
                $data['is_follow'] = 1;
            }

            $data['follow_id'] = $user_id;
            $data['followed_id'] = $friend_id;
        }
    ...
    {
        $operation_follow = htmlspecialchars($_REQUEST['operation_follow']);
        $data = array(
        ...
        if ($operation_follow == 1) {
            $ret = $this->friendModel->addFriend($data); //添加关注记录
        } else if ($operation_follow == 0) {
            $where = array(
