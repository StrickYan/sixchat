...
$context = array(
    'ssl' => array(
        'local_cert'  => '/usr/local/nginx/conf/ssl/1_sixchat.classmateer.com_bundle.crt', // 也可以是crt文件
        'local_pk'    => '/usr/local/nginx/conf/ssl/2_sixchat.classmateer.com.key',
        'verify_peer' => false,
    )
);
// PHPSocketIO服务
...
$sender_io->on('workerStart', function () {
    ...
    $inner_http_worker->onMessage = function ($http_connection, $data) {
        ...
        if ($to) {
            $sender_io->to($to)->emit(@$_POST['type'], $_POST['content']);
            // 否则向所有uid推送数据
        } else {
